test = False
import glob, speech_recognition as sr, sys, random, requests, webbrowser, json, serial, wikipediaapi, os, subprocess, pygame, datetime, torch, re, num2words
...
def speak(audio_string):
    ...
    if test == False:
        try:
            gTTS(text=audio_string, lang="en").save('output/ddbot/tts.mp3')
            audio = AudioSegment.from_mp3('output/ddbot/tts.mp3')
            try:
